...
% 
% %förbereder grafiken
plotGraph = yyaxis('left',time,data,'-o',...
    'LineWidth',1,...
    'MarkerSize',3,...
    'MarkerEdgeColor','k',...
    'MarkerFaceColor','r');
...
plotGraph2 = yyaxis('right',time,data,'-',...
    'LineWidth',1,...
    'MarkerSize',3,...
    'MarkerEdgeColor','k',...
    'MarkerFaceColor','y');
...
%while ishandle(plotGraph)   %loopar den aktiva plotten
  ...
  %inputvalue = fscanf(array2,'%f');  %läser data i floatformat
   inputvalue = (bodyAngle.*t./t);%array2; %sin(t./30).*80;
...
array2 = sin(t./30);
init = array2;
set(get(handles.maxes2, 'Parent'), 'HandleVisibility', 'on'); %set handle visibility to on
axes(handles.maxes2); %make ax1 the current axes
% 
% 
%serialPort = 'COMX'; %detta är input signal yo
danielGrafik = 'TEST:Logger Data Serial';
...
%while ishandle(plotGraph)   %loopar den aktiva plotten
  ...
  if(~isempty(inputvalue)) %ej tom och ej float
      ...
      if(scrollwidth > 0)
          ...
          %disp('2.2 ELSE LOL')
          set(plotGraph2,'XData',time,'YData',data);
          axis([0 time(count) min max]);
      end
